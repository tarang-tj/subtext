...
import { decodeMessage } from "@/lib/engines/decode";
...
export async function POST(request: Request) {
  ...
  const { message, relationship, situation } = (body ?? {}) as Record<string, unknown>;

  if (typeof message !== "string" || message.trim().length === 0) {
    return NextResponse.json(
      { error: "Paste the message you received, then press Decode." },
      { status: 400 },
    );
  }
